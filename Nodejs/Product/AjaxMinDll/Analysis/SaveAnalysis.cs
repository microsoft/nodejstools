...
namespace Microsoft.NodejsTools.Analysis {
    ...
    class SaveAnalysis {
        ...
        private object GenerateFuncRef(FunctionInfo fi) {
            string name = ".";
            for (var cd = fi.FunctionObject.Parent as ClassDefinition;
                cd != null;
        ...
        private object[] GenerateArgInfo(FunctionInfo fi, IAnalysisSet[] parameters) {
            var res = new object[Math.Min(fi.FunctionObject.Parameters.Count, parameters.Length)];
            for (int i = 0; i < res.Length; i++) {
                res[i] = GenerateParameter(fi.FunctionObject.Parameters[i], parameters[i]);
            }
            return res;
        }
