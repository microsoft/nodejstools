...
namespace Microsoft.NodejsTools.Analysis {
    ...
    class BuiltinFunctionOverloadResult : OverloadResult {
        ...
        private readonly JavaScriptAnalyzer _projectState;
        ...
        internal BuiltinFunctionOverloadResult(JavaScriptAnalyzer state, string name, IPythonFunctionOverload overload, int removedParams, Func<string> fallbackDoc, params ParameterResult[] extraParams)
            : base(null, name) {
        ...
        internal BuiltinFunctionOverloadResult(JavaScriptAnalyzer state, string name, IPythonFunctionOverload overload, int removedParams, params ParameterResult[] extraParams)
            : this(state, name, overload, removedParams, null, extraParams) {
        }

        internal BuiltinFunctionOverloadResult(JavaScriptAnalyzer state, IPythonFunctionOverload overload, int removedParams, string name, Func<string> fallbackDoc, params ParameterResult[] extraParams)
            : base(null, name) {
            _overload = overload;
            _extraParameters = extraParams;
            _removedParams = removedParams;
        ...
        internal ParameterResult GetParameterResultFromParameterInfo(IParameterInfo param) {
            ...
            if (param.ParameterTypes != null) {
                typeName = param.ParameterTypes.Where(p => p != _projectState.Types[BuiltinTypeId.Null]).Select(p => p.Name).FirstOrDefault();
            } else {
                typeName = "object";
            }
