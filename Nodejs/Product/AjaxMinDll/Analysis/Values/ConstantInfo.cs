...
namespace Microsoft.NodejsTools.Analysis.Values {
    ...
    internal class ConstantInfo : BuiltinInstanceInfo {
        ...
        public ConstantInfo(object value, JavaScriptAnalyzer projectState)
...
#if FALSE
        public ConstantInfo(IPythonConstant value, JavaScriptAnalyzer projectState)
        ...
        internal static IAnalysisSet NumericOp(Node node, BuiltinInstanceInfo lhs, AnalysisUnit unit, JSToken operation, IAnalysisSet rhs) {
            ...
            foreach(var ns in rhs) {
                ...
                if (lhsType == BuiltinTypeId.String) {
                    // TODO: NaN, if it's a number like string coerce
                    if (operation == JSToken.Plus &&
                        (rhsType == BuiltinTypeId.String)) {
                ...
                if (lhsType == BuiltinTypeId.Unknown || lhsType > BuiltinTypeId.Number || 
                    rhsType == BuiltinTypeId.Unknown || rhsType > BuiltinTypeId.Number) {
                    // Non-numeric types require the reverse operation
                    res = res.Union(ns.ReverseBinaryOperation(node, unit, operation, lhs));
                } else {
                    res = res.Union(unit.ProjectState.ClassInfos[BuiltinTypeId.Number].Instance);
                }
            }
        ...
        public override IAnalysisSet UnaryOperation(Node node, AnalysisUnit unit, JSToken operation) {
        ...
        public override IAnalysisSet Call(Node node, AnalysisUnit unit, IAnalysisSet @this, IAnalysisSet[] args) {
            return _builtinInfo.Call(node, unit, @this, args);
        }
        ...
        public override string ToString() {
            var valueStr = (_value == null) ? "" : (" '" + _value.ToString() + "'");
