...
require = function () {
    ...
    function make_module(module_name) {
        switch(module_name) { 
            ...
            case "module": return new     function module() {
                /// <summary><p>Node has a simple module loading system.  In Node, files and modules are in one-to-one correspondence.  As an example, <code>foo.js</code> loads the module <code>circle.js</code> in the same directory.  </p> <p>The contents of <code>foo.js</code>:  </p> <pre><code>var circle = require(&#39;./circle.js&#39;); console.log( &#39;The area of a circle of radius 4 is &#39;            + circle.area(4));</code></pre> <p>The contents of <code>circle.js</code>:  </p> <pre><code>var PI = Math.PI;  exports.area = function (r) {   return PI * r * r; };  exports.circumference = function (r) {   return 2 * PI * r; };</code></pre> <p>The module <code>circle.js</code> has exported the functions <code>area()</code> and <code>circumference()</code>.  To export an object, add to the special <code>exports</code> object.  </p> <p>Note that <code>exports</code> is a reference to <code>module.exports</code> making it suitable for augmentation only. If you are exporting a single item such as a constructor you will want to use <code>module.exports</code> directly instead.  </p> <pre><code>function MyConstructor (opts) {   //... }  // BROKEN: Does not modify exports exports = MyConstructor;  // exports the constructor properly module.exports = MyConstructor;</code></pre> <p>Variables local to the module will be private. In this example the variable <code>PI</code> is private to <code>circle.js</code>.  </p> <p>The module system is implemented in the <code>require(&quot;module&quot;)</code> module.  </p> </summary>
            };
            ...
            case "Events": return new     function Events() {
                ...
                this.EventEmitter = function() {
                    this.addListener = function(event, listener) {
                        /// <summary><p>Adds a listener to the end of the listeners array for the specified event.  </p> <pre><code>server.on(&#39;connection&#39;, function (stream) {   console.log(&#39;someone connected!&#39;); });</code></pre> <p>Returns emitter, so calls can be chained.  </p> </summary>
                        /// <signature>
                    ...
                    this.once = function(event, listener) {
                        /// <summary><p>Adds a <strong>one time</strong> listener for the event. This listener is invoked only the next time the event is fired, after which it is removed.  </p> <pre><code>server.once(&#39;connection&#39;, function (stream) {   console.log(&#39;Ah, we have our first user!&#39;); });</code></pre> <p>Returns emitter, so calls can be chained.  </p> </summary>
                        /// <signature>
                        /// <param name="event"></param>
                        /// <param name="listener"></param>
                        /// </signature>
                    ...
                    this.removeListener = function(event, listener) {
                        /// <summary><p>Remove a listener from the listener array for the specified event. <strong>Caution</strong>: changes array indices in the listener array behind the listener.  </p> <pre><code>var callback = function(stream) {   console.log(&#39;someone connected!&#39;); }; server.on(&#39;connection&#39;, callback); // ... server.removeListener(&#39;connection&#39;, callback);</code></pre> <p>Returns emitter, so calls can be chained.  </p> </summary>
                        /// <signature>
                        /// <param name="event"></param>
                        /// <param name="listener"></param>
                        /// </signature>
                    ...
                    this.removeAllListeners = function(event) {
                        /// <summary><p>Removes all listeners, or those of the specified event.  </p> <p>Returns emitter, so calls can be chained.  </p> </summary>
                        /// <signature>
                        /// <param name="event"></param>
                        /// </signature>
                    }
                    ...
                    this.emit = function(event, arg1, arg2) {
                        /// <summary><p>Execute each of the listeners in order with the supplied arguments.  </p> <p>Returns <code>true</code> if event had listeners, <code>false</code> otherwise.   </p> </summary>
                        /// <signature>
                        /// <param name="event"></param>
                    ...
                    /// <field name='newListener'><p>This event is emitted any time someone adds a new listener.  It is unspecified if <code>listener</code> is in the list returned by <code>emitter.listeners(event)</code>.   </p> </field>
                    this.newListener = new emitter();
                    /// <field name='removeListener'><p>This event is emitted any time someone removes a listener.  It is unspecified if <code>listener</code> is in the list returned by <code>emitter.listeners(event)</code>.  </p> </field>
                    this.removeListener = new emitter();
                }
            };
            case "domain": return new     function domain() {
                /// <summary><p>Domains provide a way to handle multiple different IO operations as a single group.  If any of the event emitters or callbacks registered to a domain emit an <code>error</code> event, or throw an error, then the domain object will be notified, rather than losing the context of the error in the <code>process.on(&#39;uncaughtException&#39;)</code> handler, or causing the program to exit immediately with an error code.  </p> </summary>
                ...
                this.Domain = function() {
                    ...
                    this.enter = function() {
                        /// <summary><p>The <code>enter</code> method is plumbing used by the <code>run</code>, <code>bind</code>, and <code>intercept</code> methods to set the active domain. It sets <code>domain.active</code> and <code>process.domain</code> to the domain, and implicitly pushes the domain onto the domain stack managed by the domain module (see <code>domain.exit()</code> for details on the domain stack). The call to <code>enter</code> delimits the beginning of a chain of asynchronous calls and I/O operations bound to a domain.  </p> <p>Calling <code>enter</code> changes only the active domain, and does not alter the domain itself. <code>Enter</code> and <code>exit</code> can be called an arbitrary number of times on a single domain.  </p> <p>If the domain on which <code>enter</code> is called has been disposed, <code>enter</code> will return without setting the domain.  </p> </summary>
                    }
                    this.exit = function() {
                        /// <summary><p>The <code>exit</code> method exits the current domain, popping it off the domain stack. Any time execution is going to switch to the context of a different chain of asynchronous calls, it&#39;s important to ensure that the current domain is exited. The call to <code>exit</code> delimits either the end of or an interruption to the chain of asynchronous calls and I/O operations bound to a domain.  </p> <p>If there are multiple, nested domains bound to the current execution context, <code>exit</code> will exit any domains nested within this domain.  </p> <p>Calling <code>exit</code> changes only the active domain, and does not alter the domain itself. <code>Enter</code> and <code>exit</code> can be called an arbitrary number of times on a single domain.  </p> <p>If the domain on which <code>exit</code> is called has been disposed, <code>exit</code> will return without exiting the domain.  </p> </summary>
                    }
            ...
            case "stream": return new     function stream() {
                ...
                this.Readable = function() {
                    ...
                    this.setEncoding = function(encoding) {
                        /// <summary><p>Call this function to cause the stream to return strings of the specified encoding instead of Buffer objects.  For example, if you do <code>readable.setEncoding(&#39;utf8&#39;)</code>, then the output data will be interpreted as UTF-8 data, and returned as strings.  If you do <code>readable.setEncoding(&#39;hex&#39;)</code>, then the data will be encoded in hexadecimal string format.  </p> <p>This properly handles multi-byte characters that would otherwise be potentially mangled if you simply pulled the Buffers directly and called <code>buf.toString(encoding)</code> on them.  If you want to read the data as strings, always use this method.  </p> <pre><code class="javascript">var readable = getReadableStreamSomehow(); readable.setEncoding(&#39;utf8&#39;); readable.on(&#39;data&#39;, function(chunk) {   assert.equal(typeof chunk, &#39;string&#39;);   console.log(&#39;got %d characters of string data&#39;, chunk.length); })</code></pre> </summary>
                        /// <signature>
                    ...
                    this.resume = function() {
                        /// <summary><p>This method will cause the readable stream to resume emitting <code>data</code> events.  </p> <p>This method will switch the stream into flowing-mode.  If you do <em>not</em> want to consume the data from a stream, but you <em>do</em> want to get to its <code>end</code> event, you can call <code>readable.resume()</code> to open the flow of data.  </p> <pre><code class="javascript">var readable = getReadableStreamSomehow(); readable.resume(); readable.on(&#39;end&#39;, function(chunk) {   console.log(&#39;got to the end, but did not read anything&#39;); })</code></pre> </summary>
                    }
                    this.pause = function() {
                        /// <summary><p>This method will cause a stream in flowing-mode to stop emitting <code>data</code> events.  Any data that becomes available will remain in the internal buffer.  </p> <p>This method is only relevant in flowing mode.  When called on a non-flowing stream, it will switch into flowing mode, but remain paused.  </p> <pre><code class="javascript">var readable = getReadableStreamSomehow(); readable.on(&#39;data&#39;, function(chunk) {   console.log(&#39;got %d bytes of data&#39;, chunk.length);   readable.pause();   console.log(&#39;there will be no more data for 1 second&#39;);   setTimeout(function() {     console.log(&#39;now data will start flowing again&#39;);     readable.resume();   }, 1000); })</code></pre> </summary>
                    }
                    this.pipe = function(destination, options) {
                        /// <summary><p>This method pulls all the data out of a readable stream, and writes it to the supplied destination, automatically managing the flow so that the destination is not overwhelmed by a fast readable stream.  </p> <p>Multiple destinations can be piped to safely.  </p> <pre><code class="javascript">var readable = getReadableStreamSomehow(); var writable = fs.createWriteStream(&#39;file.txt&#39;); // All the data from readable goes into &#39;file.txt&#39; readable.pipe(writable);</code></pre> <p>This function returns the destination stream, so you can set up pipe chains like so:  </p> <pre><code class="javascript">var r = fs.createReadStream(&#39;file.txt&#39;); var z = zlib.createGzip(); var w = fs.createWriteStream(&#39;file.txt.gz&#39;); r.pipe(z).pipe(w);</code></pre> <p>For example, emulating the Unix <code>cat</code> command:  </p> <pre><code class="javascript">process.stdin.pipe(process.stdout);</code></pre> <p>By default [<code>end()</code>][] is called on the destination when the source stream emits <code>end</code>, so that <code>destination</code> is no longer writable. Pass <code>{ end: false }</code> as <code>options</code> to keep the destination stream open.  </p> <p>This keeps <code>writer</code> open so that &quot;Goodbye&quot; can be written at the end.  </p> <pre><code class="javascript">reader.pipe(writer, { end: false }); reader.on(&#39;end&#39;, function() {   writer.end(&#39;Goodbye\n&#39;); });</code></pre> <p>Note that <code>process.stderr</code> and <code>process.stdout</code> are never closed until the process exits, regardless of the specified options.  </p> </summary>
                        /// <signature>
                    ...
                    this.unpipe = function(destination) {
                        /// <summary><p>This method will remove the hooks set up for a previous <code>pipe()</code> call.  </p> <p>If the destination is not specified, then all pipes are removed.  </p> <p>If the destination is specified, but no pipe is set up for it, then this is a no-op.  </p> <pre><code class="javascript">var readable = getReadableStreamSomehow(); var writable = fs.createWriteStream(&#39;file.txt&#39;); // All the data from readable goes into &#39;file.txt&#39;, // but only for the first second readable.pipe(writable); setTimeout(function() {   console.log(&#39;stop writing to file.txt&#39;);   readable.unpipe(writable);   console.log(&#39;manually close the file stream&#39;);   writable.end(); }, 1000);</code></pre> </summary>
                        /// <signature>
                    ...
                    this.wrap = function(stream) {
                        /// <summary><p>Versions of Node prior to v0.10 had streams that did not implement the entire Streams API as it is today.  (See &quot;Compatibility&quot; below for more information.)  </p> <p>If you are using an older Node library that emits <code>&#39;data&#39;</code> events and has a <code>pause()</code> method that is advisory only, then you can use the <code>wrap()</code> method to create a [Readable][] stream that uses the old stream as its data source.  </p> <p>You will very rarely ever need to call this function, but it exists as a convenience for interacting with old Node programs and libraries.  </p> <p>For example:  </p> <pre><code class="javascript">var OldReader = require(&#39;./old-api-module.js&#39;).OldReader; var oreader = new OldReader; var Readable = require(&#39;stream&#39;).Readable; var myReader = new Readable().wrap(oreader);  myReader.on(&#39;readable&#39;, function() {   myReader.read(); // etc. });</code></pre> </summary>
                        /// <signature>
                    ...
                    emitter = new Events().EventEmitter;
                    /// <field name='readable'><p>When a chunk of data can be read from the stream, it will emit a <code>&#39;readable&#39;</code> event.  </p> <p>In some cases, listening for a <code>&#39;readable&#39;</code> event will cause some data to be read into the internal buffer from the underlying system, if it hadn&#39;t already.  </p> <pre><code class="javascript">var readable = getReadableStreamSomehow(); readable.on(&#39;readable&#39;, function() {   // there is some data to read now })</code></pre> <p>Once the internal buffer is drained, a <code>readable</code> event will fire again when more data is available.  </p> </field>
                    this.readable = new emitter();
                    /// <field name='data'><p>If you attach a <code>data</code> event listener, then it will switch the stream into flowing mode, and data will be passed to your handler as soon as it is available.  </p> <p>If you just want to get all the data out of the stream as fast as possible, this is the best way to do so.  </p> <pre><code class="javascript">var readable = getReadableStreamSomehow(); readable.on(&#39;data&#39;, function(chunk) {   console.log(&#39;got %d bytes of data&#39;, chunk.length); })</code></pre> </field>
                    this.data = new emitter();
                    /// <field name='end'><p>This event fires when no more data will be provided.  </p> <p>Note that the <code>end</code> event <strong>will not fire</strong> unless the data is completely consumed.  This can be done by switching into flowing mode, or by calling <code>read()</code> repeatedly until you get to the end.  </p> <pre><code class="javascript">var readable = getReadableStreamSomehow(); readable.on(&#39;data&#39;, function(chunk) {   console.log(&#39;got %d bytes of data&#39;, chunk.length); }) readable.on(&#39;end&#39;, function() {   console.log(&#39;there will be no more data.&#39;); });</code></pre> </field>
                ...
                this.Writable = function() {
                    ...
                    this.end = function(chunk, encoding, callback) {
                        /// <summary><p>Call this method when no more data will be written to the stream.  If supplied, the callback is attached as a listener on the <code>finish</code> event.  </p> <p>Calling [<code>write()</code>][] after calling [<code>end()</code>][] will raise an error.  </p> <pre><code class="javascript">// write &#39;hello, &#39; and then end with &#39;world!&#39; http.createServer(function (req, res) {   res.write(&#39;hello, &#39;);   res.end(&#39;world!&#39;);   // writing more now is not allowed! });</code></pre> </summary>
                        /// <signature>
                    ...
                    emitter = new Events().EventEmitter;
                    /// <field name='drain'><p>If a [<code>writable.write(chunk)</code>][] call returns false, then the <code>drain</code> event will indicate when it is appropriate to begin writing more data to the stream.  </p> <pre><code class="javascript">// Write the data to the supplied writable stream 1MM times. // Be attentive to back-pressure. function writeOneMillionTimes(writer, data, encoding, callback) {   var i = 1000000;   write();   function write() {     var ok = true;     do {       i -= 1;       if (i === 0) {         // last time!         writer.write(data, encoding, callback);       } else {         // see if we should continue, or wait         // don&#39;t pass the callback, because we&#39;re not done yet.         ok = writer.write(data, encoding);       }     } while (i &gt; 0 &amp;&amp; ok);     if (i &gt; 0) {       // had to stop early!       // write some more once it drains       writer.once(&#39;drain&#39;, write);     }   } }</code></pre> </field>
                    this.drain = new emitter();
                    /// <field name='finish'><p>When the [<code>end()</code>][] method has been called, and all data has been flushed to the underlying system, this event is emitted.  </p> <pre><code class="javascript">var writer = getWritableStreamSomehow(); for (var i = 0; i &lt; 100; i ++) {   writer.write(&#39;hello, #&#39; + i + &#39;!\n&#39;); } writer.end(&#39;this is the end\n&#39;); write.on(&#39;finish&#39;, function() {   console.error(&#39;all writes are now complete.&#39;); });</code></pre> </field>
                    this.finish = new emitter();
                    /// <field name='pipe'><p>This is emitted whenever the <code>pipe()</code> method is called on a readable stream, adding this writable to its set of destinations.  </p> <pre><code class="javascript">var writer = getWritableStreamSomehow(); var reader = getReadableStreamSomehow(); writer.on(&#39;pipe&#39;, function(src) {   console.error(&#39;something is piping into the writer&#39;);   assert.equal(src, reader); }); reader.pipe(writer);</code></pre> </field>
                    this.pipe = new emitter();
                    /// <field name='unpipe'><p>This is emitted whenever the [<code>unpipe()</code>][] method is called on a readable stream, removing this writable from its set of destinations.  </p> <pre><code class="javascript">var writer = getWritableStreamSomehow(); var reader = getReadableStreamSomehow(); writer.on(&#39;unpipe&#39;, function(src) {   console.error(&#39;something has stopped piping into the writer&#39;);   assert.equal(src, reader); }); reader.pipe(writer); reader.unpipe(writer);</code></pre> </field>
            ...
            case "crypto": return new     function crypto() {
                ...
                this.getCiphers = function() {
                    /// <summary><p>Returns an array with the names of the supported ciphers.  </p> <p>Example:  </p> <pre><code>var ciphers = crypto.getCiphers(); console.log(ciphers); // [&#39;AES-128-CBC&#39;, &#39;AES-128-CBC-HMAC-SHA1&#39;, ...]</code></pre> </summary>
                }
                ...
                this.Cipher = function() {
                    ...
                    this.final = function(output_encoding) {
                        /// <summary><p>Returns any remaining enciphered contents, with <code>output_encoding</code> being one of: <code>&#39;binary&#39;</code>, <code>&#39;base64&#39;</code> or <code>&#39;hex&#39;</code>.  If no encoding is provided, then a buffer is returned.  </p> <p>Note: <code>cipher</code> object can not be used after <code>final()</code> method has been called.  </p> </summary>
                        /// <signature>
                        /// <param name="output_encoding"></param>
                        /// </signature>
                    }
            ...
            case "fs": return new     function fs() {
                ...
                this.openSync = function(path, flags, mode) {
                    /// <summary><p>Synchronous version of <code>fs.open()</code>.  </p> </summary>
                    /// <signature>
                    /// <param name="path"></param>
                    /// <param name="flags"></param>
                    /// <param name="mode"></param>
                ...
                this.WriteStream = function() {
                    emitter = new Events().EventEmitter;
                    /// <field name='open'><p>Emitted when the WriteStream&#39;s file is opened.  </p> </field>
                    this.open = new emitter();
                    /// <field name='bytesWritten'><p>The number of bytes written so far. Does not include data that is still queued for writing.  </p> </field>
                    this.bytesWritten = undefined;
                }
            ...
            case "path": return new     function path() {
                ...
                this.join = function(path1, path2) {
                    /// <summary><p>Join all arguments together and normalize the resulting path.  </p> <p>Arguments must be strings.  In v0.8, non-string arguments were silently ignored.  In v0.10 and up, an exception is thrown.  </p> <p>Example:  </p> <pre><code>path.join(&#39;/foo&#39;, &#39;bar&#39;, &#39;baz/asdf&#39;, &#39;quux&#39;, &#39;..&#39;) // returns &#39;/foo/bar/baz/asdf&#39;  path.join(&#39;foo&#39;, {}, &#39;bar&#39;) // throws exception TypeError: Arguments to path.join must be strings</code></pre> </summary>
                    /// <signature>
            ...
            case "http": return new     function http() {
                ...
                this.Server = function() {
                    ...
                    /// <field name='connection'><p><code>function (socket) { }</code>  </p> <p> When a new TCP stream is established. <code>socket</code> is an object of type  <code>net.Socket</code>. Usually users will not want to access this event. In  particular, the socket will not emit <code>readable</code> events because of how  the protocol parser attaches to the socket. The <code>socket</code> can also be  accessed at <code>request.connection</code>.  </p> </field>
                    this.connection = new emitter();
                ...
                this.ServerResponse = function() {
                    ...
                    emitter = new Events().EventEmitter;
                    /// <field name='close'><p><code>function () { }</code>  </p> <p>Indicates that the underlying connection was terminated before <code>response.end()</code> was called or able to flush.  </p> </field>
                    this.close = new emitter();
                ...
                /// <field name='IncomingMessage'><p>An <code>IncomingMessage</code> object is created by <code>http.Server</code> or <code>http.ClientRequest</code> and passed as the first argument to the <code>&#39;request&#39;</code> and <code>&#39;response&#39;</code> event respectively. It may be used to access response status, headers and data.  </p> <p>It implements the [Readable Stream][] interface, as well as the following additional events, methods, and properties.  </p> </field>
            ...
            case "child_process": return new     function child_process() {
                ...
                this.ChildProcess = function() {
                    ...
                    emitter = new Events().EventEmitter;
                    /// <field name='error'><p>Emitted when:  </p> <ol> <li>The process could not be spawned, or</li> <li>The process could not be killed, or</li> <li>Sending a message to the child process failed for whatever reason.</li> </ol> <p>See also <a href="#child_process_child_kill_signal"><code>ChildProcess#kill()</code></a> and <a href="#child_process_child_send_message_sendhandle"><code>ChildProcess#send()</code></a>.  </p> </field>
                    this.error = new emitter();
            ...
            case "tty": return new     function tty() {
                ...
                this.WriteStream = function() {
                    emitter = new Events().EventEmitter;
                    /// <field name='resize'><p><code>function () {}</code>  </p> <p>Emitted by <code>refreshSize()</code> when either of the <code>columns</code> or <code>rows</code> properties has changed.  </p> <pre><code>process.stdout.on(&#39;resize&#39;, function() {   console.log(&#39;screen size has changed!&#39;);   console.log(process.stdout.columns + &#39;x&#39; + process.stdout.rows); });</code></pre> </field>
                    this.resize = new emitter();
                    /// <field name='columns'><p>A <code>Number</code> that gives the number of columns the TTY currently has. This property gets updated on &quot;resize&quot; events.  </p> </field>
                    this.columns = 0;
                    /// <field name='rows'><p>A <code>Number</code> that gives the number of rows the TTY currently has. This property gets updated on &quot;resize&quot; events.  </p> </field>
                    this.rows = 0;
                }
            };
    ...
    /// <summary><p>For printing to stdout and stderr.  Similar to the console object functions provided by most web browsers, here the output is sent to stdout or stderr.  </p> <p>The console functions are synchronous when the destination is a terminal or a file (to avoid lost messages in case of premature exit) and asynchronous when it&#39;s a pipe (to avoid blocking for long periods of time).  </p> <p>That is, in the following example, stdout is non-blocking while stderr is blocking:  </p> <pre><code>$ node script.js 2&gt; error.log | tee info.log</code></pre> <p>In daily use, the blocking/non-blocking dichotomy is not something you should worry about unless you log huge amounts of data.   </p> </summary>
    ...
    this.kill = function(pid, signal) {
        /// <summary><p>Send a signal to a process. <code>pid</code> is the process id and <code>signal</code> is the string describing the signal to send.  Signal names are strings like &#39;SIGINT&#39; or &#39;SIGHUP&#39;.  If omitted, the signal will be &#39;SIGTERM&#39;. See kill(2) for more information.  </p> <p>Note that just because the name of this function is <code>process.kill</code>, it is really just a signal sender, like the <code>kill</code> system call.  The signal sent may do something other than kill the target process.  </p> <p>Example of sending a signal to yourself:  </p> <pre><code>process.on(&#39;SIGHUP&#39;, function() {   console.log(&#39;Got SIGHUP signal.&#39;); });  setTimeout(function() {   console.log(&#39;Exiting.&#39;);   process.exit(0); }, 100);  process.kill(process.pid, &#39;SIGHUP&#39;);</code></pre> <p>Note: SIGUSR1 is reserved by node.js.  It can be used to kickstart the debugger.  </p> </summary>
        /// <signature>
    ...
    /// <field name='stdout'><p>A <code>Writable Stream</code> to <code>stdout</code>.  </p> <p>Example: the definition of <code>console.log</code>  </p> <pre><code>console.log = function(d) {   process.stdout.write(d + &#39;\n&#39;); };</code></pre> <p><code>process.stderr</code> and <code>process.stdout</code> are unlike other streams in Node in that writes to them are usually blocking.  They are blocking in the case that they refer to regular files or TTY file descriptors. In the case they refer to pipes, they are non-blocking like other streams.  </p> <p>To check if Node is being run in a TTY context, read the <code>isTTY</code> property on <code>process.stderr</code>, <code>process.stdout</code>, or <code>process.stdin</code>:  </p> <pre><code>$ node -p &quot;Boolean(process.stdin.isTTY)&quot; true $ echo &quot;foo&quot; | node -p &quot;Boolean(process.stdin.isTTY)&quot; false  $ node -p &quot;Boolean(process.stdout.isTTY)&quot; true $ node -p &quot;Boolean(process.stdout.isTTY)&quot; | cat false</code></pre> <p>See <a href="tty.html#tty_tty">the tty docs</a> for more information.  </p> </field>
    this.stdout = undefined;
    ...
    this.version = undefined;
    /// <field name='versions'><p>A property exposing version strings of node and its dependencies.  </p> <pre><code>console.log(process.versions);</code></pre> <p>Will print something like:  </p> <pre><code>{ http_parser: &#39;1.0&#39;,   node: &#39;0.10.4&#39;,   v8: &#39;3.14.5.8&#39;,   ares: &#39;1.9.0-DEV&#39;,   uv: &#39;0.10.3&#39;,   zlib: &#39;1.2.3&#39;,   modules: &#39;11&#39;,   openssl: &#39;1.0.1e&#39; }</code></pre> </field>
    ...
    this.pid = 0;
    /// <field name='title'><p>Getter/setter to set what is displayed in &#39;ps&#39;.  </p> <p>When used as a setter, the maximum length is platform-specific and probably short.  </p> <p>On Linux and OS X, it&#39;s limited to the size of the binary name plus the length of the command line arguments because it overwrites the argv memory.  </p> <p>v0.8 allowed for longer process title strings by also overwriting the environ memory but that was potentially insecure/confusing in some (rather obscure) cases.   </p> </field>
    this.title = '';
