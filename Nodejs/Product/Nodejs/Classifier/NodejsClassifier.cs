...
namespace Microsoft.NodejsTools.Classifier {
    ...
    internal class NodejsClassifier : IClassifier {
            ...
            while (currentLine <= lastLine) {
                ...
                for (int i = 0; i < lineTokenization.Tokens.Length; i++) {
                    ...
                    if (token.Category == TokenCategory.IncompleteMultiLineStringLiteral) {
                        ...
                        if (i == 0) {
                            length += GetLeadingMultiLineStrings(JSScanner, snapshot, firstLine, currentLine, out validPrevLine, ref startToken);
                        } else {
                            validPrevLine = currentLine;
                        }

                        if (i == lineTokenization.Tokens.Length - 1) {
                            length += GetTrailingMultiLineStrings(JSScanner, snapshot, currentLine, state);
                        }
        ...
        private int GetLeadingMultiLineStrings(JSScanner JSScanner, ITextSnapshot snapshot, int firstLine, int currentLine, out int validPrevLine, ref TokenInfo startToken) {
            validPrevLine = currentLine;
        ...
        private int GetTrailingMultiLineStrings(JSScanner JSScanner, ITextSnapshot snapshot, int currentLine, object state) {
