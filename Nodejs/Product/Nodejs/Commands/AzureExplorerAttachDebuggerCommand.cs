...
namespace Microsoft.NodejsTools.Commands {
    ...
    internal class AzureExplorerAttachDebuggerCommand : Command {
        ...
        private async Task<XDocument> GetPublishXml(AzureWebSiteInfo webSiteInfo) {
            ...
            var webSiteServices = (IVsAzureWebSiteServices)NodejsPackage.GetGlobalService(typeof(IVsAzureWebSiteServices));
            ...
            var subscriprions = await webSiteService.GetSubscriptionsAsync();
            var subscription = subscriprions.FirstOrDefault(s => s.SubscriptionId == webSiteInfo.SubscriptionId);
            if (subscription == null) {
                return null;
            }

            var webSites = await subscription.GetWebSitesAsync(false);
            var webSite = webSites.FirstOrDefault(w => {
                Uri browseUri;
                Uri.TryCreate(w.BrowseURL, UriKind.Absolute, out browseUri);
            ...
            var requestUri = new Uri(
                subscription.ServiceManagementEndpointUri,
                string.Format("{0}/services/WebSpaces/{1}/sites/{2}/publishxml", webSite.Subscription.SubscriptionId, webSite.WebSpace, webSite.Name));
            ...
            var certCtx = subscription.AzureCredentials as IAzureAuthenticationCertificateSubscriptionContext;
            if (certCtx != null) {
                var cert = await certCtx.AuthenticationCertificate.GetCertificateFromStoreAsync();
                request.ClientCertificates.Add(cert);
            } else {
                var accountCtx = subscription.AzureCredentials as IAzureUserAccountSubscriptionContext;
                if (accountCtx == null) {
                    return null;
                }
                string authHeader = await accountCtx.GetAuthenticationHeaderAsync(false);
